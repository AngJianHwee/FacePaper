...
def train_task(task, run_suffix, device):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(os.path.join(task_dir, f'training_{task}_vitsmall.log'))
        ]
    )
    ...
    # Initialize ViTSmall model
    model = ViTSmall(num_classes=n_classes).to(device)
    print(f"Model: ViTSmall for {task}")
    ...
    for epoch, i in enumerate(range(1, num_epochs + 1), 1):
        ...
        if val_loss < best_val_loss:
            ...
            val_model_path = os.path.join(task_dir, 'best_model_val_vitsmall.pth')
        ...
        if test_loss < best_test_loss:
            ...
            test_model_path = os.path.join(task_dir, 'best_model_test_vitsmall.pth')
