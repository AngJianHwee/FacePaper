...
def get_unique_suffix():
    ...
    return f"{timestamp}_{unique_id}_no_att_disease_age_5"
...
def train_epoch(model, train_loader, optimizer, criterion, device, current_epoch, total_epochs):
    ...
    correct = {'age_5': 0, 'disease': 0}
    total = {'age_5': 0, 'disease': 0}
...
def test(model, test_loader, criterion, device, current_epoch=None, total_epochs=None):
    ...
    all_predictions = {'age_5': [], 'disease': []}
    all_targets = {'age_5': [], 'disease': []}
...
def main():
    ...
    for epoch, i in enumerate(range(1, num_epochs + 1), 1):
        ...
        print(f'[{i}/{num_epochs}] Train Loss: {train_loss:.4f}, Train Acc - Age: {train_acc["age_5"]:.2f}%, Disease: {train_acc["disease"]:.2f}%')
        print(f'[{i}/{num_epochs}] Val Loss: {val_loss:.4f}, Val Acc - Age: {val_acc["age_5"]:.2f}%, Disease: {val_acc["disease"]:.2f}%')
        print(f'[{i}/{num_epochs}] Test Loss: {test_loss:.4f}, Test Acc - Age: {test_acc["age_5"]:.2f}%, Disease: {test_acc["disease"]:.2f}%')
        
        logging.info(f'[{i}/{num_epochs}] Epoch completed in {epoch_time:.2f}s')
        logging.info(f'[{i}/{num_epochs}] Train Loss: {train_loss:.4f}, Train Acc - Age: {train_acc["age_5"]:.2f}%, Disease: {train_acc["disease"]:.2f}%')
        logging.info(f'[{i}/{num_epochs}] Val Loss: {val_loss:.4f}, Val Acc - Age: {val_acc["age_5"]:.2f}%, Disease: {val_acc["disease"]:.2f}%')
        logging.info(f'[{i}/{num_epochs}] Test Loss: {test_loss:.4f}, Test Acc - Age: {test_acc["age_5"]:.2f}%, Disease: {test_acc["disease"]:.2f}%')
