...
class AttentionMobileNetShallow_s_single_task(nn.Module):
    def __init__(self, input_channels, n_classes, input_size=224, use_attention=False, attention_channels=64):
        super(AttentionMobileNetShallow_s_single_task, self).__init__()
    ...
    def forward(self, x, return_att_map=False, return_latent=False):
        ...
        if return_att_map:
            if return_latent:
                return out, att_map, x_att, latent
            else:
                return out, att_map, x_att
        else:
            if return_latent:
                return out, latent
            else:
